...
def aggregate_eval_results(summary,
    ...
    for cat in cats:
        ...
        for metric in metrics:
            ...
            new_res.append(res)
        if len(cats) == 24:
            ds_name = {'1': 'person', '2': 'car', '3': 'motorcycle', '4': 'truck', '5': 'bird', '6': 'dog', '7': 'handbag', '8': 'suitcase', '9': 'bottle', '10': 'cup', '11': 'bowl', '12': 'chair',
                        '13': 'potted plant', '14': 'bed', '15': 'dining table', '16': 'tv', '17': 'laptop', '18': 'cell phone', '19': 'bag', '20': 'bin', '21': 'box', '22': 'door',
                        '23': 'road barrier', '24': 'stick'}
        elif len(cats) == 8:
            ds_name = {'1': "pedestrian", '2': "rider", '3': "car", '4': "truck", '5': "bus", '6': "train", '7': "motorcycle", '8': "bicycle"}

        new_summary.loc[str(cat) + " " + ds_name["{}".format(cat)]] = new_res
...
def eval_mot(anns, all_results, split_camera=False, class_average=False, a_or_v=None):
    print('Evaluating BDD Results...')

    if a_or_v is not None:
        if a_or_v == "v":
            for i in range(len(anns['annotations'])):
                anns['annotations'][i]['segmentation'] = anns['annotations'][i]['visible_mask']
        else:
            for i in range(len(anns['annotations'])):
                anns['annotations'][i]['segmentation'] = anns['annotations'][i]['amodal_mask']
    ...
    before_drop, res = preprocessResult(all_results, anns, cats_mapping)
    print('mot.py: EVAL_MOT Results before drop:', sum([len(i) for i in before_drop]))
    print('mot.py: EVAL_MOT Results after drop:', sum([len(i) for i in res]))
