        ...
        for key in results.get('mask_fields', []):
            ...
            if self.keep_ratio:
                if isinstance(results[key], list):
                    results[key] = [results[key][i].rescale(results['scale']) for i in range(2)]
                else:
                    results[key] = results[key].rescale(results['scale'])
            else:
                if isinstance(results[key], list):
                    results[key] = [results[key][i].resize(results['img_shape'][:2]) for i in range(2)]
                else:
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('mask_fields', []):
                if isinstance(results[key], list):
                    results[key] = [results[key][i].flip(results['flip_direction']) for i in range(2)]
                else:
...
class Pad(object):
    ...
    def _pad_masks(self, results):
        ...
        for key in results.get('mask_fields', []):
            if isinstance(results[key], list):
                results[key] = [results[key][i].pad(pad_shape, pad_val=self.pad_val) for i in range(2)]
            else:
...
class Expand(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('mask_fields', []):
            if isinstance(results[key], list):
                results[key] = [results[key][i].expand(
                    int(h * ratio), int(w * ratio), top, left) for i in range(2)]
            else:
